...
const RoomDetailsPage: React.FC = () => {
    ...
    const { roomId } = useParams<{ roomId: string }>(); // Get room ID from URL parameters
    const [roomDetails, setRoomDetails] = useState<Room | null>(null);
    const [isLoading, setIsLoading] = useState<boolean>(true); // Track loading state
    const [error, setError] = useState<string | null>(null); // Track any errors
    const [checkInDate, setCheckInDate] = useState<Date | null>(null); // State variable for check-in date
    const [checkOutDate, setCheckOutDate] = useState<Date | null>(null); // State variable for check-out date
    const [numAdults, setNumAdults] = useState<number>(1); // State variable for number of adults
    const [numChildren, setNumChildren] = useState<number>(0); // State variable for number of children
    const [totalPrice, setTotalPrice] = useState<number>(0); // State variable for total booking price
    const [totalGuests, setTotalGuests] = useState<number>(1); // State variable for total number of guests
    const [showDatePicker, setShowDatePicker] = useState<boolean>(false); // State variable to control date picker visibility
    const [userId, setUserId] = useState<string>(''); // Set user id
    const [showMessage, setShowMessage] = useState<boolean>(false); // State variable to control message visibility
    const [confirmationCode, setConfirmationCode] = useState<string>(''); // State variable for booking confirmation code
    const [errorMessage, setErrorMessage] = useState<string>(''); // State variable for error message
        ...
        <div className="room-details-booking">
            {showMessage && (
                <p className="booking-success-message">
                    Booking successful! Confirmation code: {confirmationCode}. An SMS and email of your booking details
                ...
                {showDatePicker && (
                    <div className="date-picker-container">
                        <DatePicker
                            ...
                            selectsStart
                            startDate={checkInDate || undefined} // Chuyển đổi null thành undefined
                            endDate={checkOutDate || undefined} // Chuyển đổi null thành undefined
                        ...
                        <DatePicker
                            ...
                            startDate={checkInDate || undefined} // Chuyển đổi null thành undefined
                            endDate={checkOutDate || undefined} // Chuyển đổi null thành undefined
                            minDate={checkInDate || undefined} // Chuyển đổi null thành undefined
